 ...
 */
function captureTab(): Promise<string> { // Aggiunto tipo tabId e ritorno Promise<string>
    ...
    return new Promise((resolve, reject) => {
        ...
        chrome.tabs.captureVisibleTab(chrome.windows.WINDOW_ID_CURRENT, { format: "png" }, (imageDataUrl?: string) => { // Aggiunto tipo opzionale per imageDataUrl
 ...
 */
async function saveData(key: string, data: SavedCaptureData): Promise<void> { // Tipi per key, data e ritorno
    try {
        // Usa la funzione tipizzata da storage.util
        await storageSet({ [key]: data });
        console.log(`Web Area Saver: Dati salvati con chiave ${key}.`);
    } catch (error: any) { // Tipo 'any' o 'Error' per l'errore catturato
 ...
 */
function notifyContentScript(tabId: number, type: string, message: string): void { // Tipi per parametri, ritorno void
    // La payload del messaggio ora corrisponde a MessagePayload (senza 'data')
    const payload: MessagePayload = { type, message };
    chrome.tabs.sendMessage(tabId, payload)
        .catch((e: Error) => console.warn(`Web Area Saver: Impossibile inviare messaggio a tab ${tabId} (forse chiusa o senza content script attivo?): ${e.message}`));
}
 ...
 */
function extractUserErrorMessage(error: Error | any): string { // Tipo Error | any per flessibilità
    // Usa la funzione tipizzata da storage.util
 ...
 */
async function handleCaptureRequest(selectionData: SelectionData, sender: chrome.runtime.MessageSender): Promise<void> { // Tipi per parametri, ritorno Promise<void>
    ...
    } catch (error: any) { // Tipo 'any' per catturare tutti gli errori possibili
...
chrome.action.onClicked.addListener(async (tab: chrome.tabs.Tab) => { // Tipo chrome.tabs.Tab per il parametro tab
    if (tab.id) {
